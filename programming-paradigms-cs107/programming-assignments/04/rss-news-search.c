...
static void BuildIndices(const char *feedsFileName, hashset *stopWords, hashset *indexedWords);
static void ProcessFeed(const char *remoteDocumentName, hashset *stopWords, hashset *indexedWords);
static void PullAllNewsItems(urlconnection *urlconn, hashset *stopWords, hashset *indexedWords);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *stopWords, hashset *indexedWords);
static void ExtractElement(streamtokenizer *st, const char *htmlTag, char dataBuffer[], int bufferLength);
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL, hashset *stopWords, hashset *indexedWords);
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *unused, const char *articleURL, hashset *stopWords, hashset *indexedWords);
static void QueryIndices(hashset *stopWords, hashset *indexedWords);
static void ProcessResponse(const char *word, hashset *stopWords, hashset *indexedWords);
static bool WordIsWellFormed(const char *word, hashset *stopWords);
...
static int ArticleCompare(const void *aAddr1, const void *aAddr2)
{
    article *a1 = (article *) aAddr1;
    article *a2 = (article *) aAddr2;

    // printf("ArticleCompare: \"%s\" \"%s\"\n", a1->link, a2->link);
    // Articles should have different links.
    return strcasecmp(a1->link, a2->link);
}
...
/* Strings */
static int StringHash(const void *sAddr, int numBuckets)
{            
    char *s = *(char **) sAddr;
...
static int WordKeyHash(const void *addr, int numBuckets)
{
    wordKey *wk = (wordKey *) addr;
    return StringHash(&wk->key, numBuckets);
}

static int WordKeyCompare(const void *elemAddr1, const void *elemAddr2)
{
    wordKey *wk1 = (wordKey *) elemAddr1;
    wordKey *wk2 = (wordKey *) elemAddr2;

    int result = strcasecmp(wk1->key, wk2->key);
    // printf("WordKeyCompare: \"%s\" \"%s\" = %d\n", wk1->key, wk2->key, result);
    return result;
}

static void WordKeyFree(void *elemAddr)
{
    wordKey *wk = elemAddr;
    free(wk->key);
    VectorDispose(&wk->articles);
}
...
static const char *const kWelcomeTextFile = "/home/d1r3ct/learning/university-courses/stanford-programming-paradigms/programming-assignments/04/assn-4-rss-news-search-data/welcome.txt";
static const char *const kDefaultFeedsFile = "/home/d1r3ct/learning/university-courses/stanford-programming-paradigms/programming-assignments/04/assn-4-rss-news-search-data/adam-learns.txt";
static const char *const stopWordsFile = "./data/stop-words.txt";

void PrintChar(void *elemAddr, void *auxData)
{
    printf("PrintChar: %s\n", *(char **) elemAddr);
}
...
int main(int argc, char **argv)
{
    ...
    // Tokenize the contents of the file with newline as a delimeter.
    streamtokenizer stopWords;
    STNew(&stopWords, infile, "\n", true);
    char word[128];

    // Create a hashset for the stop words.
    hashset stopWordsHash;
    HashSetNew(&stopWordsHash, sizeof(char *), 1009, StringHash, StringCompare, StringFree);

    // Save all the stop words into the hashset.
    while (STNextToken(&stopWords, word, sizeof(word))) {
        // I need to allocate memory for the word dynamically.
        char *wordPtr = strdup(word);
        HashSetEnter(&stopWordsHash, &wordPtr);
    }

    // Create a hashset for the word occurrences and lists of their corresponding articles.
    hashset wordsInArticles;
    HashSetNew(&wordsInArticles, sizeof(wordKey), 1009, WordKeyHash, WordKeyCompare, WordKeyFree);

    Welcome(kWelcomeTextFile); 
    BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &stopWordsHash, &wordsInArticles);
    QueryIndices(&stopWordsHash, &wordsInArticles);

    HashSetDispose(&stopWordsHash);
...
static void BuildIndices(const char *feedsFileName, hashset *stopWords, hashset *indexedWords)
{
    ...
    while (STSkipUntil(&st, ":") != EOF) { // ignore everything up to the first selicolon of the line
        ...
        ProcessFeed(remoteFileName, stopWords, indexedWords);
    }
        ...
        hashset *indexedWords)
{
    ...
    switch (urlconn.responseCode) {
        case 0: printf("Unable to connect to \"%s\".  Ignoring...", u.serverName);
                break;
        case 200: PullAllNewsItems(&urlconn, stopWords, indexedWords);
                  break;
        case 301: 
        case 302: ProcessFeed(urlconn.newUrl, stopWords, indexedWords);
                  break;
...
static void PullAllNewsItems(urlconnection *urlconn, hashset *stopWords, hashset *indexedWords)
{
    ...
    while (GetNextItemTag(&st)) {
        // if true is returned, then assume that <item ...> has just been read
        // and pulled from the data stream
        ProcessSingleNewsItem(&st, stopWords, indexedWords);
    }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *stopWords, hashset *indexedWords)
{
    ...
    while (GetNextTag(st, htmlTag, sizeof(htmlTag)) && (strcasecmp(htmlTag, kItemEndTag) != 0)) {
        // TODO: I might need a different way of extracting information between the tags.
    ...
    ParseArticle(articleTitle, articleDescription, articleURL, stopWords, indexedWords);
}
...
static void ParseArticle(const char *articleTitle, const char *articleDescription,
        const char *articleURL, hashset *stopWords, hashset *indexedWords)
{
    url u;
    urlconnection urlconn;
    streamtokenizer st;
    ...
    switch (urlconn.responseCode) {
        ...
        case 200: printf("Scanning \"%s\" from \"http://%s\"\n", articleTitle, u.serverName);
                  STNew(&st, urlconn.dataStream, kTextDelimiters, false);
                  ScanArticle(&st, articleTitle, articleDescription, articleURL, stopWords, indexedWords);
                  STDispose(&st);
                  break;
        case 301:
        case 302: // just pretend we have the redirected URL all along, though index using the new URL and not the old one...
                  ParseArticle(articleTitle, articleDescription, urlconn.newUrl, stopWords, indexedWords);
                  break;
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
        const char *unused, const char *articleURL, hashset *stopWords,
        hashset *indexedWords)
{
    int numWords = 0;
    char word[1024];
    ...
    wordKey *found;
    wordKey localWordKey;

    article currentArticle;
    currentArticle.title = articleTitle;
    currentArticle.link = articleURL;
    currentArticle.occurrences = 1;

    while (STNextToken(st, word, sizeof(word))) {
        // Collect an article info.
        ...
        if (strcasecmp(word, "<") == 0) {
            ...
            if (WordIsWellFormed(word, stopWords)) {
                numWords++;

                if (strlen(word) > strlen(longestWord))
                    strcpy(longestWord, word);

                localWordKey.key = word;
                wordKey *found = (wordKey *) HashSetLookup(indexedWords, &localWordKey);

                if (found == NULL) {
                    // When encountering a new word, add it to the hashset with an array of a single article.
                    localWordKey.key = strdup(word);

                    VectorNew(&localWordKey.articles, sizeof(article), ArticleFree, 4);
                    currentArticle.title = strdup(articleTitle);
                    currentArticle.link = strdup(articleURL);
                    VectorAppend(&localWordKey.articles, &currentArticle);

                    HashSetEnter(indexedWords, &localWordKey);
                } else {
                    // If this word is already indexed.

                    // We add the current article to the list.
                    int articleIndex = VectorSearch(&found->articles, &currentArticle, ArticleCompare, 0, false);

                    article *ca;
                    if (articleIndex == -1) {
                        currentArticle.title = strdup(articleTitle);
                        currentArticle.link = strdup(articleURL);
                        VectorAppend(&found->articles, &currentArticle);
                        articleIndex = VectorLength(&found->articles) - 1;
                    }

                    ca = (article *) VectorNth(&found->articles, articleIndex);

                    // And update the n of occurrences.
                    ca->occurrences += 1;
                }
            }
        }
    ...
    printf("\tWe counted %d well-formed words [including duplicates].\n", numWords); printf("\tThe longest word scanned was \"%s\".", longestWord);
...
static void QueryIndices(hashset *stopWords, hashset *indexedWords)
{
    ...
    while (true) {
        ...
        ProcessResponse(response, stopWords, indexedWords);
    }
}
...
{
    wordKey localWk;
    localWk.key = strdup(word);

    if (WordIsWellFormed(word, stopWords)) {
        // First we need to pull up the wordKey by the key from the hashset.
        wordKey *wk = (wordKey *) HashSetLookup(indexedWords, &localWk);

        if (wk != NULL) {
            printf("Number of articles: %d\n", VectorLength(&wk->articles));
            for (int i = 0; i < 10 && i < VectorLength(&wk->articles); ++i) {
                article *a = (article *) VectorNth(&wk->articles, i);
                printf("%d.) \"%s\" [search term \"%s\" occurs %d times] \"%s\"\n", i, a->title, wk->key, a->occurrences, a->link);
            }
        } else {
...
static bool WordIsWellFormed(const char *word, hashset *stopWords)
{
    ...
    // Make sure that the word is not in the list of stop-words. Do we just do
    // linear search? I would like to utilize compare function.
