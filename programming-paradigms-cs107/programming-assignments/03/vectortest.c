...
static void TestSearch(vector *v, char ch)
{
  ...
  // TODO: Add test that ensures we receive the correct index from the search
  // function.
  printf("\n`foundSorted` is %d, and `foundNot` is %d", foundSorted, foundNot);
...
static void TestInsertDelete(vector *alphabet)
{
    ...
    fprintf(stdout, "\nAfter adding an exlamation mark to the end: ");
    VectorMap(alphabet, PrintChar, stdout);
...
static void MemoryTest()
{
    ...
    for (i = 0; i < kNumQuestionWords; i++) {
        ...
        // why the ampersand? isn't `questionWord` already a pointer?
    }
