...
static void ProcessFeed(rssFeedState *state, const char *remoteDocumentName);
static void PullAllNewsItems(rssFeedState *state, urlconnection *urlconn);
...
static void ParseArticle(rssFeedState *state, const char *articleTitle, const char *articleURL);
...
static const char *const kDefaultStopWordsFile = "/home/d1r3ct/learning/university-courses/stanford-programming-paradigms/programming-assignments/04/assn-4-rss-news-search-data/stop-words.txt";
static const char *const kDefaultFeedsFile = "/home/d1r3ct/learning/university-courses/stanford-programming-paradigms/programming-assignments/04/assn-4-rss-news-search-data/rss-feeds-techy.txt";
// static const char *const kDefaultFeedsFile = "/home/d1r3ct/learning/university-courses/stanford-programming-paradigms/programming-assignments/04/assn-4-rss-news-search-data/rss-feeds-tiny.txt";
...
static void BuildIndices(rssDatabase *db, const char *feedsFileURL)
{
    ...
    rssFeedState state = {db};
    state.availableConnections = SemaphoreNew("Available connections", 8);

    streamtokenizer st;
    char remoteFileName[2048];
    HashSetNew(&state.db->indices, sizeof(rssIndexEntry), kNumIndexEntryBuckets, IndexEntryHash, IndexEntryCompare, IndexEntryFree);
    VectorNew(&state.db->previouslySeenArticles, sizeof(rssNewsArticle), NewsArticleFree, 0);
    STNew(&st, infile, kNewLineDelimiters, true);

    while (STSkipUntil(&st, ":") != EOF) { // ignore everything up to the first selicolon of the line
        STSkipOver(&st, ": ");		   // now ignore the semicolon and any whitespace directly after it
        STNextToken(&st, remoteFileName, sizeof(remoteFileName));   
        ProcessFeed(&state, remoteFileName);
    }

    RunAllThreads();

    printf("\n");
    SemaphoreFree(state.availableConnections);
    STDispose(&st);
    fclose(infile);
}
...
static void ProcessFeed(rssFeedState *state, const char *remoteDocumentName)
{
    ...
    switch (urlconn.responseCode) {
        case 0: {
            printf("Unable to connect to Ignoring...");
            printf("Unable to connect to \"%s\".  Ignoring...", u.serverName);
            break;
        }
        case 200: {
            PullAllNewsItems(state, &urlconn);
            break;
        }
        case 301: 
        case 302: {
            ProcessFeed(state, urlconn.newUrl);
            break;
        }
        default: {
...
static void PullAllNewsItems(rssFeedState *state, urlconnection *urlconn)
{
    ...
    // Go through all the news articles, pull their contents and index them.
...
{
    rssFeedState *state = userData;
    // Since this is yet a single thread, this memory location will get
    // overwritten when we branch out. We can keep track of all the items with
    // an extra hashset. We will copy the items into the hashset. Or I can
    // allocate this dynamically, I will have the strings on the heap and I
    // will be able to pass them further.
    char *title = strdup(state->entry.title);
    char *url = strdup(state->entry.url);

    state->entry.activeField = NULL;
    if (strcasecmp(name, "item") == 0) {
        ThreadNew(title, ParseArticle, 4, state, title, url);
    }
}
...
static void ParseArticle(rssFeedState *state, const char *articleTitle, const char *articleURL)
{
    ...
    streamtokenizer st;
    int articleID;
    bool didBailOut = false;

    URLNewAbsolute(&u, articleURL);
    rssNewsArticle newsArticle = { articleTitle, u.serverName, u.fullName };
    if (VectorSearch(&state->db->previouslySeenArticles, &newsArticle, NewsArticleCompare, 0, false) >= 0) {
        printf("[Ignoring \"%s\": we've seen it before.]\n", articleTitle);
        URLDispose(&u); 
        return; 
    }

    SemaphoreWait(state->availableConnections);
    ...
    switch (urlconn.responseCode) {
        ...
        case 200:
            ...
            VectorAppend(&state->db->previouslySeenArticles, &newsArticle);
            articleID = VectorLength(&state->db->previouslySeenArticles) - 1;
            STNew(&st, urlconn.dataStream, kTextDelimiters, false);
            ScanArticle(&st, articleID, &state->db->indices, &state->db->stopWords);
            STDispose(&st);
            );
            break;
        case 301: 
        case 302:
            didBailOut = true;
            // Signal that the current connection is done.
            SemaphoreSignal(state->availableConnections);
            // Just pretend we have the redirected URL all along, though index using the new URL and not the old one...
            ParseArticle(state, articleTitle, urlconn.newUrl);
            break;
        default:
